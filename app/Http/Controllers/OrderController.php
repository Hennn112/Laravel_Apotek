...
class OrderController extends Controller
{
    ...
    {
        $key = $request->date;
        $orders = Order::with('user')->where('created_at','LIKE',"%$key%")->orwhere('name_customer', 'LIKE', "%$key%")
                                     ->simplePaginate(10);
    ...
    public function store (Request $request){
        ...
        foreach ($arrayDistinct as $id => $count) {
            ...
            if ($medicine->stock <= $count) {
                return redirect()->back()->with('failed', 'stock sedang kosong');
            }else{
                $result = $medicine['stock'] -= $count;
    ...
    public function downloadPDF($id){
        ...
        // mengirim inisial variable dari data yang akan digunakan pada layouts pdf
